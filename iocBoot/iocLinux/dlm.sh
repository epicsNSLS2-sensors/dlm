...
# Manually set IOC_STARTUP_DIR if dlm.sh will reside somewhere other than iocdlm
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/dlm/iocBoot/iocdlm
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select dlm, but not dlm.sh
...
start() {
	...
	screenpid
        ...
        cd ${IOC_STARTUP_DIR}
	# Run dlm inside a screen session
...
run() {
	...
	screenpid
        ...
        ${ECHO} "Starting ${IOC_NAME}"
        cd ${IOC_STARTUP_DIR}
	# Run dlm outside of a screen session, which is helpful for debugging
	${IOC_CMD}
    fi
}
